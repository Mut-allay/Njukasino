...
describe("Full Multiplayer Gameplay Loop", () => {
  ...
  beforeEach(() => {
    // 1. Signup/Login Hero (full signup + onboarding flow - can be slow)
    cy.signupAndLoginTestUser();

    // Set player name for create lobby
    cy.get('input[placeholder*="name"]').type("TestPlayer");

    // 2. Seed Bots in Firestore (requires backend on localhost:8000)
    cy.request({
      method: "POST",
      url: "http://localhost:8000/simulation/seed-bots",
  ...
  function playTurns() {
    ...
    cy.get("body").then(($body) => {
      if (
        $body.find(".game-over-modal").length > 0 ||
        $body.find('[data-testid="game-over-modal"]').length > 0
      ) {
        ...
        if (playerName.includes("(You)")) {
          ...
          cy.request("POST", `http://localhost:8000/game/${gameId}/draw`).then(
            () => {
              ...
              cy.request(
                "POST",
                `http://localhost:8000/game/${gameId}/discard?card_index=0`,
              ).then(() => {
